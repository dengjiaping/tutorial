...
/**
 * 
 * @ClassName: LockSetupActivity
 * @Description: 设置手势密码界面
 * @author mio4kon
 * @mail mio4kon.dev@gmail.com
 * @date 2014-10-16 上午10:36:44
 * 
 */
	...
	public void onClick(View v) {
		...
		case R.id.right_btn:
			if (step == STEP_2) {
				...
				PreferencesUtils.putString(mContext, ConstantValue.GESTURE_KEY,
						LockPatternView.patternToString(choosePattern));
				/** 这是设置手势密码成功,可以提示一下,进入手势输入界面 */
				final LStyleDialog lStyleDialog = new LStyleDialog(mContext);
				lStyleDialog.setTitle("提示").setMessage("手势密码设置成功").setPositiveButton("ok", new OnClickListener() {
