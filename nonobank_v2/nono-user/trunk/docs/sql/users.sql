-- ------------------
-- 用户系统数据库表结构
-- $Id: users.sql 300 2014-10-29 08:32:39Z yichuan $
...
-- ---------------------------------
-- Table structure for USER_CRIMINAL
-- ---------------------------------
DROP TABLE IF EXISTS `USER_CRIMINAL`;
CREATE TABLE `USER_CRIMINAL` (
  `ID` BIGINT(20) NOT NULL AUTO_INCREMENT,
  `USER_ID` BIGINT(20) NOT NULL COMMENT '用户ID',
  `USER_NAME` VARCHAR(50) NOT NULL COMMENT '用户名',
  `CRIMINAL_STATUS` VARCHAR(20) NULL DEFAULT NULL COMMENT '状态(A B 无 查无此人 处理中)',
  `CRIMINAL_INFO` VARCHAR(200) NULL DEFAULT NULL COMMENT '刑侦记录',
  `LAST_MODIFIED_DATE` TIMESTAMP NULL DEFAULT CURRENT_TIMESTAMP ON UPDATE CURRENT_TIMESTAMP COMMENT '更新时间',
  `CREATED_DATE` TIMESTAMP NOT NULL DEFAULT CURRENT_TIMESTAMP COMMENT '创建时间',
  PRIMARY KEY (`ID`),
  INDEX `USER_ID` (`USER_ID`)
) COMMENT='用户刑侦记录表' ENGINE=InnoDB DEFAULT CHARSET=utf8;