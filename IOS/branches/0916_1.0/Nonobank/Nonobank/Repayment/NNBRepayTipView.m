...
@implementation NNBRepayTipView
...
//
//    NNBRepayTipView *_maskview = self;
...
- (IBAction)sureOk:(id)sender {
    [self hide];
    if(_delegate && [_delegate respondsToSelector:@selector(isSureRepay:)])
    {
        [_delegate isSureRepay:YES];
    }
...
-(void)hide
{
    [UIView animateWithDuration:0.3 animations:^{
        self.alpha = 0.0;
        [self.contentView.layer addAnimation:[NNBRepayTipView scaleAnimation:FALSE] forKey:@"LOGINVIEWWILLDISAPPEAR"];
    } completion:^(BOOL finished){
